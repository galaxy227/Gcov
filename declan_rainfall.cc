...
extern "C" {
	int asm_max_element(int *vec, int size);
}
...
void Rainfall::print_max_rainfall(vector<int>& rainyDays) {
	...
	else cout << asm_max_element(rainyDays.data(), rainyDays.size()) << endl; // This increases speed by a whopping 1% 
}
...
void Rainfall::write_benchmark(const int version, const float timer, vector<int>& totalDays) {
	fstream file("benchmark.txt", fstream::out|fstream::app);
